...
/// 保存到当前工具执行上下文的 workspace/downloads/ 下，AI Agent 可通过文件系统工具看到和管理。
///
/// 注册方式：
/// ```swift
...
public struct DownloadFileTool: ClientTool {

    public let name = "download_file"
    public let description = "Download any file (image, video, audio, document, archive, etc.) to the workspace downloads/ directory"
    ...
    public func execute(
        ...
        let downloadsDir = try workspace.resolve(relativePath: "downloads")
        ...
        let saveURL = try workspace.resolve(relativePath: relativePath, createParentDirectory: true)
    ...
    private func resolveFileName(
        ...
        if let custom = customName, !custom.isEmpty {
            guard custom == (custom as NSString).lastPathComponent,
                  custom != ".", custom != ".." else {
                throw ToolError.invalidArguments("fileName must be a file name, not a path")
            }
...
private struct DownloadFileResult: Encodable {
    let success: Bool
